...
class TimeProcessor:
    ...
    def add_time_column(magnet_data: MagnetData) -> None:
        ...
        try:
            ...
            raise DataFormatError(
                f"Failed to convert Date column in {magnet_data.filename}"
            )
        ...
        try:
            ...
            raise DataFormatError(
                f"Failed to convert Time column in {magnet_data.filename}"
            )
        ...
        try:
            ...
            raise DataFormatError(
                f"Failed to create timestamp column in {magnet_data.filename}"
            )
    ...
    def _find_and_remove_duplicates(data: pd.DataFrame, filename: str) -> pd.DataFrame:
        ...
        if duplicates.any():
            print(
                f"Warning: Found {duplicates.sum()} duplicate timestamps in {filename}"
            )
    ...
    def _validate_time_intervals(data: pd.DataFrame, filename: str) -> None:
        ...
        if dt.min() != dt.max():
            print(
                f"Warning: Inconsistent time intervals in {filename}: {dt.min()} to {dt.max()}"
            )
