...
@click.option(
    "--formula", help='Add new column with formula (e.g., "Power = Field * Current")'
)
@click.option(
    "--compute", is_flag=True, help="Compute derived quantities (e.g., water density)"
)
...
@click.option(
    "--key-pairs", multiple=True, help='Key pairs to plot (format: "key1;key2")'
)
...
@click.option("--housing", default="M9", help="Housing type")
@click.option(
    "--keys", multiple=True, help="Keys to filter (if not specified, uses all keys)"
)
...
def filter(ctx, files, housing, keys, min_value, max_value, exclude_zeros, sample_rate):
    ...
    for file_path in files:
        ...
        try:
            ...
            click.echo(
                f"    Filtered from {original_length} to {len(filtered_data)} rows"
            )
...
def _add_formula(
    magnet_data, formula, file_path, plot_formula, vs_time, normalize, save
):
...
def _plot_key_pairs(magnet_data, key_pairs, file_path, save):
    ...
    DataPlotter.plot_xy_pairs_to_file(magnet_data, key_pairs, file_path, save, not save)
