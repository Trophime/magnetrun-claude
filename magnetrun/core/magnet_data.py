...
class MagnetData:
    ...
    def __init__(self, data_handler: BaseData, format_name: str, field_registry: Optional[FormatRegistry] = None):
    ...
    @classmethod
    def from_file(cls, filepath: Union[str, Path], field_config: Optional[Union[str, Path]] = None) -> 'MagnetData':
        ...
        if format_name in ["bprofile", "pupitre"]:
            ...
            raise FileFormatError(f"Handler creation not implemented for format: {format_name}")
        ...
        if field_config:
            try:
                custom_format = FormatDefinition.load_from_file(field_config, field_registry.ureg)
    ...
    @classmethod
    def from_pandas(cls, filename: str, data: pd.DataFrame, format_type: str = "pupitre", 
                   field_registry: Optional[FormatRegistry] = None) -> 'MagnetData':
    ...
    @classmethod
    def from_dict(cls, filename: str, groups: dict, keys: list, data: dict,
                 format_type: str = "pigbrother", field_registry: Optional[FormatRegistry] = None) -> 'MagnetData':
    ...
    def add_data(self, key: str, formula: str, field_type: Optional[str] = None, 
                unit: Optional[str] = None, symbol: Optional[str] = None) -> None:
    ...
    def convert_field_values(self, key: str, values: Union[List[float], np.ndarray, pd.Series], 
        ...
        if isinstance(values, pd.Series):
            converted = field.convert_values(values.tolist(), target_unit, self._field_registry.ureg)
    ...
    def _get_field_coverage(self) -> float:
        ...
        defined_count = sum(1 for key in self.keys if self._format_def.get_field(key) is not None)
    ...
    def validate_field_data(self, key: str) -> dict:
        ...
        try:
            ...
            if validation_result['invalid_values'] == 0 and len(validation_result['issues']) == 0:
                validation_result['status'] = 'valid'
            elif validation_result['invalid_values'] < validation_result['valid_values']:
    ...
    def get_field_validation_summary(self) -> dict:
        ...
        valid_fields = sum(1 for r in validation_results.values() if r.get('status') == 'valid')
        mostly_valid_fields = sum(1 for r in validation_results.values() if r.get('status') == 'mostly_valid')
        invalid_fields = sum(1 for r in validation_results.values() if r.get('status') in ['invalid', 'error'])
        undefined_fields = sum(1 for r in validation_results.values() if r.get('status') == 'no_field_definition')
        ...
        return {
            'summary': {
                ...
                'coverage_percent': ((total_fields - undefined_fields) / total_fields * 100) if total_fields > 0 else 0,
                'quality_percent': ((valid_fields + mostly_valid_fields) / total_fields * 100) if total_fields > 0 else 0
            },
    ...
    def load_format_definition(self, filepath: Union[str, Path]) -> None:
        ...
        loaded_format = FormatDefinition.load_from_file(filepath, self._field_registry.ureg)
    ...
    def auto_validate_units(self) -> dict:
        ...
        for field_name in self._format_def.list_fields():
            validation_results[field_name] = self._format_def.validate_field_unit(field_name)
        
        # Summary
        valid_units = sum(1 for r in validation_results.values() if r.get('valid', False))
        total_fields = len(validation_results)
        ...
        return {
            'summary': {
                ...
                'validity_percent': (valid_units / total_fields * 100) if total_fields > 0 else 0
            },
            'field_results': validation_results
        }
