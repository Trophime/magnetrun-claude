...
class ConfigPaths:
    ...
    # Default paths
    DEFAULT_BASE_DIR = Path(__file__).parent / "../config/configs"
...
class ConfigManager:
    ...
    def load_config(
        self, config_type: str, config_name: str, use_cache: bool = True
    ) -> Optional[Dict]:
    ...
    def save_config(
        self, config_type: str, config_name: str, config_data: Dict
    ) -> bool:
    ...
    def get_config_info(self) -> Dict:
        ...
        info = {
            ...
            "custom_dirs": {
                name: str(path) for name, path in paths.custom_dirs.items()
            },
            "directories_exist": {
        ...
        for name in paths.custom_dirs:
            info["directories_exist"][f"custom_{name}"] = paths.custom_dirs[
                name
            ].exists()
    ...
    def create_default_configs(self, overwrite: bool = False) -> Dict[str, bool]:
        ...
        for format_name, config in default_formats.items():
            ...
            if not config_path.exists() or overwrite:
                results[f"format_{format_name}"] = self.save_config(
                    "format", format_name, config
                )
            else:
                results[f"format_{format_name}"] = (
                    False  # Already exists, not overwritten
                )
        ...
        for housing_name, config in default_housings.items():
            ...
            if not config_path.exists() or overwrite:
                results[f"housing_{housing_name}"] = self.save_config(
                    "housing", housing_name, config
                )
            else:
...
def setup_config_from_env():
    ...
    if base_dir:
        ...
        if os.getenv("MAGNETRUN_FIELD_DEFS_DIR"):
            config_paths.field_definitions_dir = Path(
                os.getenv("MAGNETRUN_FIELD_DEFS_DIR")
            )
