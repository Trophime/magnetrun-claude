...
class HousingConfig:
                ...
                for format_type in DataFormat:
                    ...
                    if format_key in magnet_data:
                        ...
                        self.magnet_configs[magnet_type][format_type] = {
                            'groups': self._parse_groups(format_config.get('groups', {})),
                            'aliases': format_config.get('aliases', {}),
                            'computed_fields': self._parse_computed_fields(format_config.get('computed_fields', {})),
                            'checks': self._parse_checks(format_config.get('checks', {}))
                        }
    ...
    def _parse_computed_fields(self, computed_data: Dict[str, Any]) -> Dict[str, ComputedFieldConfig]:
    ...
    def get_magnet_config(self, magnet_type: MagnetType, data_format: DataFormat) -> Optional[Dict[str, Any]]:
        """Get configuration for a specific magnet type and data format."""
        return self.magnet_configs.get(magnet_type, {}).get(data_format)

    def set_magnet_config(self, magnet_type: MagnetType, data_format: DataFormat, config: Dict[str, Any]) -> None:
    ...
    def get_groups(self, magnet_type: MagnetType, data_format: DataFormat) -> Dict[str, GroupConfig]:
    ...
    def get_aliases(self, magnet_type: MagnetType, data_format: DataFormat) -> Dict[str, str]:
        """Get field aliases for a magnet type and format."""
        config = self.get_magnet_config(magnet_type, data_format)
        return config.get('aliases', {}) if config else {}

    def get_computed_fields(self, magnet_type: MagnetType, data_format: DataFormat) -> Dict[str, ComputedFieldConfig]:
        """Get computed fields for a magnet type and format."""
        config = self.get_magnet_config(magnet_type, data_format)
        return config.get('computed_fields', {}) if config else {}

    def get_checks(self, magnet_type: MagnetType, data_format: DataFormat) -> Dict[str, CheckConfig]:
    ...
    def get_field_info(self, field_name: str, magnet_type: MagnetType, data_format: DataFormat) -> Dict[str, Any]:
...
class HousingConfigManager:
    ...
    def _get_default_config_dir(self) -> str:
        ...
        return str(current_dir / "configs" / "housing")
    ...
    def save_configuration(self, config: HousingConfig, housing_name: Optional[str] = None) -> bool:
    ...
    def create_configuration(self, housing_name: str, metadata: Optional[Dict[str, Any]] = None) -> HousingConfig:
...
def get_housing_config_manager(config_directory: Optional[str] = None) -> HousingConfigManager:
...
def get_housing_config(housing_name: str) -> Optional[HousingConfig]:
    ...
    return manager.get_configuration(housing_name)
